...
<nav class="app-width mx-auto mt-8 flex select-none flex-row items-center justify-between p-4">
	...
	<!-- Desktop -->
	<div class="hidden flex-row items-center justify-end gap-2 px-4 sm:flex">
		{#each Object.entries(routes) as [url, name]}
			<a
				class="text-xl
        {(url === '' ? $page.url.pathname === '/' : $page.url.pathname === '/' + url)
	...
	<div
		class="absolute top-4 z-20 flex flex-col rounded bg-slate-800 p-2
    {popup ? 'right-2 opacity-100' : '-right-44 opacity-0'} transition-all"
	>
		{#each Object.entries(routes) as [url, name]}
			<a
				class="text-xl
